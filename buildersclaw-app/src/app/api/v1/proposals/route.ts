...
import {
  getContractPrizePool,
  getUsdcDecimals,
  getUsdcSymbol,
  verifySponsorFunding,
} from "@/lib/chain";
  ...
  try {
    ...
    if (!rateCheck.allowed) {
      return NextResponse.json(
        {
          success: false,
          error: { message: "Too many proposal submissions. Try again later." },
        },
        { status: 429 },
      );
    }
    ...
    const hackathonConfig: Record<string, unknown> = {
      ...
      min_participants: Math.max(
        2,
        Math.min(500, Number(body.hackathon_min_participants) || 5),
      ),
      team_size_max: Math.max(
        1,
        Math.min(20, Number(body.hackathon_team_size_max) || 5),
      ),
      challenge_type: sanitize(body.challenge_type, 50) || "other",
      contract_address: sanitize(body.contract_address, 128),
      chain_id: Number.isInteger(Number(body.chain_id))
    ...
    if (fundingTxHash && hackathonConfig.contract_address) {
        ...
        return NextResponse.json(
          {
            ...
            error: {
              message:
                "sponsor_wallet is required when funding_tx_hash is provided",
            },
          },
          { status: 400 },
        );
      ...
      try {
        ...
        hackathonConfig.funding_amount_units =
          funding.prizePoolUnits.toString();
        ...
        return NextResponse.json(
          {
            success: false,
            error: {
              message: `Funding verification failed: ${verifyErr instanceof Error ? verifyErr.message : "unknown error"}`,
            },
          },
          { status: 400 },
        );
    ...
    if (
      ...
      !hackathonConfig.brief ||
      !hackathonConfig.deadline
    ) {
      return NextResponse.json(
        {
          success: false,
          error: {
            message:
              "hackathon_title, hackathon_brief, and hackathon_deadline are required",
          },
        },
        { status: 400 },
      );
    ...
    if (!company || !email || !problem || !track) {
      return NextResponse.json(
        {
          success: false,
          error: { message: "company, email, track, and problem are required" },
        },
        { status: 400 },
      );
    }
    ...
    if (insertErr) {
      ...
      return NextResponse.json(
        {
          success: false,
          error: { message: "Failed to submit proposal. Try again." },
        },
        { status: 500 },
      );
    }
    ...
    if (judgeKey) {
      ...
      responseData.judge_skill_url = "https://buildersclaw.xyz/judge-skill.md";
      responseData.judge_instructions =
        "Save this judge API key NOW — it will NOT be shown again. It activates when your hackathon is approved. Tell your judge agent to read the judge-skill.md for instructions.";
    }
...
export async function GET(req: NextRequest) {
  ...
  if (queryErr) {
    return NextResponse.json(
      { success: false, error: { message: "Query failed" } },
      { status: 500 },
    );
  }
  ...
  try {
    ...
    if (!id || !newStatus || !["approved", "rejected"].includes(newStatus)) {
      return NextResponse.json(
        {
          success: false,
          error: { message: "id and status (approved|rejected) required" },
        },
        { status: 400 },
      );
    }
    ...
    if (proposal.status !== "pending") {
      return NextResponse.json(
        {
          success: false,
          error: { message: `Proposal already ${proposal.status}` },
        },
        { status: 409 },
      );
    }
    ...
    if (newStatus === "approved" && proposal.hackathon_config) {
      ...
      if (cfg.title && cfg.brief && cfg.deadline) {
        ...
        const deadlineStr = cfg.deadline.includes("T")
          ? cfg.deadline +
            (cfg.deadline.includes("+") ||
            cfg.deadline.includes("-", 10) ||
            cfg.deadline.endsWith("Z")
              ? ""
              : "-03:00")
          : cfg.deadline;
          ...
          return NextResponse.json(
            {
              success: false,
              error: {
                message: `Invalid deadline date: "${cfg.deadline}". Cannot create hackathon.`,
              },
            },
            { status: 400 },
          );
        ...
        if (cfg.funding_verified && cfg.contract_address) {
          try {
            const balanceUnits = await getContractPrizePool(
              ...
              return NextResponse.json(
                {
                  ...
                  error: {
                    message:
                      "Escrow contract has no funds. Sponsor may have called abort().",
                  },
                },
                { status: 400 },
              );
            ...
            prizePool = Number(
              formatUnits(
                balanceUnits,
                cfg.token_decimals || getUsdcDecimals(),
              ),
            );
            ...
            return NextResponse.json(
              {
                success: false,
                error: {
                  message: `Failed to verify contract funds: ${chainErr instanceof Error ? chainErr.message : "unknown error"}`,
                },
              },
              { status: 400 },
            );
        ...
        const judgingCriteria = serializeHackathonMeta({
          ...
          token_decimals:
            typeof cfg.token_decimals === "number"
              ? cfg.token_decimals
              : getUsdcDecimals(),
        ...
        if (insertErr) {
          console.error(
            "Auto hackathon creation failed:",
    ...
    const { error: updateErr } = await supabaseAdmin
      ...
      .update({
        ...
        admin_notes:
          sanitize(body.notes, 2000) ||
          (hackathonId ? `Hackathon auto-created: ${hackathonId}` : null),
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        status: hackathonId ? "hackathon_created" : newStatus,
        ...(hackathonId
          ? { hackathon_id: hackathonId, hackathon_url: hackathonUrl }
          : {}),
        ...(hackathonId &&
        proposal.hackathon_config &&
        typeof (proposal.hackathon_config as { contract_address?: string })
          .contract_address === "string"
          ? {
              contract_address: (
                proposal.hackathon_config as { contract_address?: string }
              ).contract_address,
            }
          : {}),
      },
